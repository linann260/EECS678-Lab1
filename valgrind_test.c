...
int main() {
  ...
  for (uninitialized_variable = 0; uninitialized_variable < 100; uninitialized_variable++) {
    ...
    *definitely_lost = (void*) malloc(7); // Give the pointer something else to
    ...
    // Free memory while still in scope.
    free(*definitely_lost); // Free memory that *definitely_lost points to. - Anna
    free(definitely_lost);  // Free memory allocated for the pointer. - Anna
  }
  ...
  free(still_reachable); // Free still_reachable. - Anna
  
  possibly_lost = malloc(10);
  void* temp = possibly_lost; // Create another pointer that points to possibly_lost. - Anna
  ...
  free(temp); // Free the pointer that points to possible_lost. - Anna
