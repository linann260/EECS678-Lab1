...
struct Storage {
  ...
  char* sentence[7]; // It might be necessary to grow this array.
                     ...
                     // Array grew from 6 to 7 to add NULL to the end of 
                     // bug_info.sentence to prevent a segmentation fault. - Anna
...
void echoohce(char** strs) {
  ...
  for (iter = strs; *iter != NULL; ++iter)
    printf("%s ", *iter); // Garbage characters were printed since "%s" and iter are not compatable types (determined using ptype).
                          // "%s" is a char type (single array) while iter is a string (2D array).
                          // So to use "%s", we need to dereference iter by adding a wildcard: *iter. - Anna
  ...
  // wrong.
  for (iter; iter != stop_beginning; --iter) // Change loop condition to iter != stop_beginning to make sure the loop stops
                                             // when we have reached the beginning of bug_info.sentence. iter is not deferenced
                                             // because we want to compare the memory address of the pointer rather than the content. - Anna

                                             // Original loop had a uninitialized variable that was detected by valgrind.
                                             // So we declared iter inside the loop. - Anna
    printf("%s ", *iter);
...
int main(int argc, char** argv) {
  ...
  bug_info.num_bugs_on_mars = 0; // Originally uninitialized variable that was caught by valgrind. - Anna
  ...
  bug_info.sentence[6] = NULL; // NULL is added to the array to fix the one of the segnmentation fault
                               // by correctly terminating the recursive echo function when NULL is found. - Anna
  ...
  free(bug_info.sentence[2]); // Since we want to change the reference of the pointer, we first need to free it.
  ...
  // Set all pointers to NULL after deallocating memory. (null) can now be printed.
  // Note: When freeing up memory for a pointer, it is important to set the pointer to NULL. - Anna
  ...
  // Removed free(bug_info.useless_bugs) which resolved the segmentation fault.
  // bug_info.useless_bugs was not allocated so it cannot be deallocated.
